...
import (
	...
	jsonmerge2 "gomodules.xyz/jsonpatch/v2"
...
type UpdatingObjectManager[Type client.Object] struct {
	...
	// TrackDesired when true, the desired resource is tracked after creation before updates and on delete errors.
	TrackDesired bool

	// HarmonizeImmutableFields allows fields that are immutable on the current
	// object to be copied to the desired object to avoid creating
	...
	// loss if the resource's server representation contains fields that do not exist on the duck-typed object.
...
// Setup is deprecated, use SetupWithManager instead.
...
func (r *UpdatingObjectManager[T]) SetupWithManager(ctx context.Context, mgr ctrl.Manager, bldr *builder.Builder) error {
	...
	if r.TrackDesired {
		...
		bldr.Watches(ct, reconcilers.EnqueueTracked(ctx))
	}
...
func (r *UpdatingObjectManager[T]) Manage(ctx context.Context, resource client.Object, actual, desired T) (T, error) {
	...
	pc := reconcilers.RetrieveOriginalConfigOrDie(ctx)
	c := reconcilers.RetrieveConfigOrDie(ctx)
	if r.DangerouslyAllowDuckTypes {
		c.Client = duck.NewDuckAwareClientWrapper(c)
		ctx = reconcilers.StashConfig(ctx, c)
	}

	if (IsNil(actual) || actual.GetCreationTimestamp().Time.IsZero()) && IsNil(desired) {
		if err := reconcilers.ClearFinalizer(ctx, resource, r.Finalizer); err != nil {
			return nilT, err
		}
		return nilT, nil
	}
	...
	if err := reconcilers.AddFinalizer(ctx, resource, r.Finalizer); err != nil {
		return nilT, err
	}

	// create the resource if it doesn't exist
	if IsNil(actual) || actual.GetCreationTimestamp().Time.IsZero() {
	...
	// look up and apply remote mutations
	...
	log.Info("updating resource", "diff", cmp.Diff(r.sanitize(actual), r.sanitize(current), reconcilers.IgnoreAllUnexported))
...
func NewPatch(base, update client.Object) (*Patch, error) {
	...
	patch, err := jsonmerge2.CreatePatch(baseBytes, updateBytes)
	if err != nil {
		return nil, err
	}
